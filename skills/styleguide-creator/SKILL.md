...
- Extend the existing `DesignSystem` and small helper extensions before creating a new styling framework.
- Treat `bjjtracker/DesignSystem/` as the primary home for shared style tokens.
- Treat `bjjtracker/Helpers/Extensions/Color/Color.swift` as a valid place for SwiftUI-facing `Color` helpers if the work is color-only.
...
- For colors, prefer SwiftUI `Color` accessors backed by asset colors. Add UIKit accessors only where UIKit APIs require them.
...
- For fonts, define reusable SwiftUI `Font` and matching `UIFont` helpers only for styles that repeat. Avoid wrapping every one-off modifier.
...
3. Add or update centralized accessors in `DesignSystem` or focused helper files.
